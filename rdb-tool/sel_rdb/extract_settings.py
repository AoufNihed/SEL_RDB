...
SEL_FID_EXPRESSION=r'^FID=([\w :+/\\()!,.\-_\\*]{10,})\r\n'
SEL_PARTNO_EXPRESSION=r'^PARTNO=([\w :+/\\()!,.\-_\\*]{10,})\r\n'
SEL_BFID_EXPRESSION=r'^BFID=([\w :+/\\()!,.\-_\\*]{10,})\r\n'
...
def return_file_paths(args_path, file_extension):
    ...
    fpath = args_path[0].replace('"', '')
    # If the path is a file, return it directly
    if os.path.isfile(fpath):
        return [fpath]
    # If the path is a directory, find files inside
    elif os.path.isdir(fpath):
        return findfiles('*.' + file_extension.lower(), fpath)
    else:
...
def process_rdb_files(files_to_do, args):
    ...
    # Determine headers and grouping size
    if args.settings is not None:
        headers = ['filename'] + args.settings
        group_size = len(args.settings)
    else:
        # If settings not provided, infer headers from parameter_info
        if parameter_info:
            # Assume each entry in parameter_info is [filename, param1, param2, ...]
            headers = ['filename'] + [f'param_{i}' for i in range(1, len(parameter_info[0]))]
            group_size = len(parameter_info[0])
        else:
            headers = ['filename']
            group_size = 1
    ...
    grouped = grouper(parameter_info, group_size, '')

    for file_data in grouped:
        # Build row and pad/trim to match header length
        row = [file_data[0][0]] + [k[-1] for k in file_data]
...
def extract_parameters(filename, rdb_info, args):
    ...
    if args.settings is not None:
        for k in args.settings:
            parameter_list.append(k.replace(r'"', ''))
    else:
        # If no settings provided, extract all available parameters
        # This assumes rdb_info contains all streams and their settings
        for stream in rdb_info:
            settings_name = str(stream[0][1])
            parameter_list.append(settings_name)

    # iterate across all parameters the user specified or all found
    for parameter in parameter_list:
        ...
        if parameter.find(PARAMETER_SEPARATOR) != -1:
            category_file_list = \
                SEL_FILES_TO_GROUP.get((parameter.split(PARAMETER_SEPARATOR))[0], None)
